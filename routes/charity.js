...
router.post('/order/cancel/:id', isCharityManager, async (req, res) => {
    try {
        ...
        // Find the order and check if it belongs to the charity
        ...
        if (!order) {
            req.flash('error_msg', 'Order not found or cannot be cancelled');
            return res.redirect('/charity/dashboard');
        }

        // Update order status to cancelled
        order.status = 'cancelled';
        order.cancellationReason = reason || 'Cancelled by charity';
        await order.save();

        // Return food items to inventory
